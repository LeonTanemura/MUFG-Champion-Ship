...
from ..gbm import LightGBMRegressor, XGBoostRegressor, CatBoostRegressor
...
class XGBLGBMCATRegressor(BaseRegressor):
    def __init__(self, input_dim, output_dim, model_config, verbose, seed=None) -> None:
        ...
        self.xgb_model = XGBoostRegressor(input_dim, output_dim, model_config=self.model_config.xgboost, verbose=0, seed=seed)
        self.lgbm_model = LightGBMRegressor(input_dim, output_dim, model_config=self.model_config.lightgbm, verbose=0, seed=seed)
        self.cat_model = CatBoostRegressor(input_dim, output_dim, model_config=self.model_config.catboost, verbose=0, seed=seed)
    ...
    def predict(self, X):
        ...
        l = 0.395
        c = 1-x-l
        return (self.xgb_model.predict(X)*x + self.lgbm_model.predict(X)*l + self.cat_model.predict(X)*c)
